...
static void vicon_callback(const geometry_msgs::TransformStamped::ConstPtr& msg)
{
  ...
  kf.processUpdate(dt);
  const KalmanFilter::Measurement_t meas(msg->transform.translation.x, msg->transform.translation.y, msg->transform.translation.z);
  ...
  */
  for(int i = 0; i < 6; i++)
  {
    odom_msg.pose.covariance[7*i] = 1e-5;
    odom_msg.twist.covariance[7*i] = 1e-5;
  }   
  ...
  Eigen::Matrix3d R(Eigen::Quaterniond(msg->transform.rotation.w, msg->transform.rotation.x, msg->transform.rotation.y, msg->transform.rotation.z));
  if(dt > 1e-6)
  {
