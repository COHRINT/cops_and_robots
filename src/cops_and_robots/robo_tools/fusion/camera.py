...
class Camera(Sensor):
    ...
    def __init__(self,robot_pose=(0,0,0),visible=True,
        ...
        #Set the unencumbered viewcone. self.viewcone represents the actual view
        self.ideal_viewcone = MapObj('Ideal viewcone',viewcone_pts,
                                     visible=False,default_color_str='pink',pose=robot_pose,
                                     has_zones=False,centroid_at_origin=False) 
        self.viewcone = MapObj('Viewcone',viewcone_pts,
                                     visible=True,default_color_str='lightyellow',pose=robot_pose,
                                     has_zones=False,centroid_at_origin=False)
    ...
    def move_viewcone(self,robot_pose):
        ...
        self.ideal_viewcone.move_shape(transform,rotation_pt=self.view_pose[0:2])
        self.viewcone.move_shape(transform,rotation_pt=self.view_pose[0:2])
        self.view_pose = pose
        print(('Camera view pose is',pose))
    ...
    def rescale_viewcone(self,robot_pose,shape_layer):
        ...
        if self.viewcone.shape.intersects(all_shapes):
            ...
            self.viewcone.shape = affinity.scale(self.ideal_viewcone.shape,xfact=scale,
                                        yfact=scale,origin=self.view_pose[0:2])
        else:
            self.viewcone.shape = self.ideal_viewcone.shape
          
    def detect(self,fusion_engine_type,particles):
        if fusion_engine_type is 'discrete':
...
if __name__ == '__main__':
    ...
    for point in goal_points:
        ...
        kinect.viewcone.plot(plot_zones=False,color=cnames['yellow'],alpha=0.5)  
