...
class Planner(object):
    ...
    def find_goal_pose(self,fusion_engine=None,):
        ...
        if self.type is 'simple':
            target_pose = self.find_goal_simply()
        elif self.type is 'particle':
            if not fusion_engine:
                raise ValueError('A fusion_engine object must be provided.')
        ...
        if self.use_target_as_goal:
            ...
            print("New goal: {}".format(["{:.2f}".format(a) for a in self.goal_pose]))
        else:
            self.goal_pose = self.view_goal(target_pose)
            print("New view goal: {} to see {}" \
    ...
    def update_path(self,current_pose,goal_pose=None):
        ...
        self.path_theta = math.atan2(goal_pose[1] - current_pose[1],goal_pose[0] - current_pose[0]) #[rad]
    ...
    def view_goal(self,target_pose):
        ...
        view_circle = Point(target_pose[0:2]).buffer(self.view_distance).exterior

        #Find any feasible point on the view_circle 
        feasible_arc = self.feasible_layer.pose_region.intersection(view_circle)
        pt = feasible_arc.representative_point()
        theta = math.atan2(pt.y - target_pose[1],pt.y - target_pose[0]) #[rad]
    ...
    def find_goal_from_particles(self,fusion_engine):
        ...
        :rtype: list of floats.
        """
        if not fusion_engine.particle_filter:
